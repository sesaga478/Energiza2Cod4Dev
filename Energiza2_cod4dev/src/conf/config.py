class ConfigETL(object):
    cfg = dict()
    @classmethod
    def init_config(cls):
        Config = {}
        workspace = {'data_raw_path': '../../data/raw_data/',
                     'data_clean_path': '../../data/clean_data/'}

        output_data_train = {'file_name_historico_consumo': 'eegsa_dataconsumo_consolidado_clean.parquet',
                'file_name_historico_consumo' : 'eegsa_dataconsumo_consolidado_clean.parquet',
                'file_name_historico_ordenes' : 'eegsa_dataordenes_consolidado_clean.parquet',
                'file_name_extra_data' : 'eegsa_dataextra_consolidado_clean.parquet',
                'file_name_data_train' : 'eegsa_wide_train_data.parquet'
                }

        output_data_prediction = {
                             'file_name_data_prediction': 'eegsa_wide_evaluacion_data.parquet'
                             }
        Config['workspace'] = workspace
        Config['output_data_train'] = output_data_train
        Config['output_data_prediction'] = output_data_prediction
        ##--FECHA DEL ULTIMO CONSUMO EVALUADO POR EL MODELO--##
        Config['mes_ultimo_consumo'] = '2022-07-01'
...
class Config(object):
    ...
    def init_config(cls):
        Config = {}
        workspace = {'data_path' : '../../data/clean_data/',
